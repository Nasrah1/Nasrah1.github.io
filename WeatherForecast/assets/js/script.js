...
function getWeatherInfo(name) {
  if (name != "") {
    var queryURL =
      ...
      name +
      "&units=imperial" +
      "&APPID=de59a8f8c6f83fcd05e3f53322f0807b";
    ...
    $.ajax({
      ...
      .then(function (response) {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          $(cardBody).append(
            `<p class="card-text1.1">Humidity: ${humidity}</>`
          );
          $(cardBody).append(`<p class="card-text1.1">${date}</>`);
          $(cardBody).append(
            `<img src="http://openweathermap.org/img/w/${icon}.png" alt="Weather Icon" class="card-text1.1">`
          );
